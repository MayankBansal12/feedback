...
export async function GET(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        response = { success: true, status: 200, message: "fetched form data!", data: form };
        return new Response(JSON.stringify(response), { status: response.status })
    } catch (error) {
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        const form = await FormModel.findByIdAndUpdate({ _id: params.id }, { name, heading })
        response = { success: true, status: 200, message: "updated form data!", data: form ?? {} };
        return new Response(JSON.stringify(response), { status: response.status })
    } catch (error) {
        if (error instanceof z.ZodError) {
            response = { success: false, status: 400, message: error.errors[0].message };
            ...
            response = { success: false, status: 400, message: "Error updating a form!" };
        }
        return new Response(JSON.stringify(response), { status: response.status })
    }
}
